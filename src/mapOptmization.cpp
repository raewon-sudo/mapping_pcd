...
void xoa_file(std::string saveNodeBINDirectory, std::string curr_scd_node_idx)
{
    int unused = system((std::string("exec rm -r ") + saveNodeBINDirectory + curr_scd_node_idx + ".bin").c_str());
}
...
public:
    mapOptimization()
    {
        ...
        writing_file_save(saveProcessDirectory);

        saveSCDDirectory = saveProcessDirectory + "SCDs/"; // SCD: scan context descriptor
        writing_file_save(saveSCDDirectory);

        saveNodePCDDirectory = saveProcessDirectory + "Scans/";
        writing_file_save(saveNodePCDDirectory);

        saveNodeBINDirectory = saveProcessDirectory + file_BIN;
        writing_file_save(saveNodeBINDirectory);

        std::string file_direction = "/Downloads/TEST/";
        BIN_save = std::getenv("HOME") + file_direction;
        writing_file_save(BIN_save);
    ...
    void gpsHandler(const sensor_msgs::NavSatFixConstPtr &gpsTopicMsgIn)
    {
        ...
        bool good_gps = (gpsTopicMsgIn->status.status != sensor_msgs::NavSatStatus::STATUS_NO_FIX &&
                         !std::isnan(gpsTopicMsgIn->altitude) &&
                         !std::isnan(gpsTopicMsgIn->latitude) &&
                         !std::isnan(gpsTopicMsgIn->longitude) &&
                         gpsTopicMsgIn->position_covariance[0] <= gpsCovThreshold &&
                         gpsTopicMsgIn->position_covariance[4] <= gpsCovThreshold &&
                         gpsTopicMsgIn->position_covariance[8] <= gpsCovThreshold_z);
    ...
    bool saveMapService(mapping_pcd::save_mapRequest &req, mapping_pcd::save_mapResponse &res)
    {
        ...
        else
        {
            ...
            writing_file_save(saveMapDirectory);
        }
    ...
    // RSLoop sử dụng khoảng cách theo Euclid
    ...
    bool detectLoopClosureDistance(int *latestID, int *closestID)
    {
        ...
        if (loopKeyPre == -1 || loopKeyCur == loopKeyPre) // Kiểm tra tất cả các Key từ LoopKeyPre đến LoopkeyCur phù hợp với 1 khoảng cách
            return false;
...
#pragma omp parallel for num_threads(numberOfCores)
        ...
        while (!gpsTopicQueue.empty())
        {
            ...
            else
            {
                ...
                if (map_project == projectmapType::UTM)
                {
                    ...
                    Eigen::Vector3d xyz_utm(gps_utm.easting, gps_utm.northing, gps_utm.altitude);
                ...
                else if (map_project == projectmapType::MGRS)
                {
                    ...
                    gps_point.ele = thisTopicGPS.position.altitude;
                    ;
    ...
    void addGPS2Factor()
    {
        ...
        while (!gpsTopicQueue.empty())
        {
            ...
            else
            {
                ...
                float gps_x = gps_mgrs.x() - zero_projector->x();
                float gps_y = gps_mgrs.y() - zero_projector->y();
                float gps_z = gps_mgrs.z() - zero_projector->z();
    ...
    void saveKeyFramesAndFactor()
    {
        ...
        {
            addGPS2Factor();
            ROS_INFO_STREAM("\033[1;32m -- Khong su dung Navsat_transform "
                            << "\033[0m");
        }
        ...
        saveBINfile(BIN_save + curr_scd_node_idx + ".bin", thisKeyFrameCloud);
    ...
    void delete_file()
    {
        ...
        {
            rate.sleep();
            xoa_file(saveNodeBINDirectory, curr_scd_node_idx);
        }
    }
};
...
int main(int argc, char **argv)
{
    ...
    // std::thread delete_file(&mapOptimization::delete_file, &MO);
