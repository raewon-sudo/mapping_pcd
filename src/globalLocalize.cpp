...
public:
	...
	//std::cout << "come in" << std::endl;
        ...
        pubOdomAftMappedROS = nh.advertise<nav_msgs::Odometry> ("mapping_pcd/mapping/odometry", 1);
        pubPath = nh.advertise<nav_msgs::Path>("mapping_pcd/mapping/path", 1);

        subLaserCloudInfo = nh.subscribe<mapping_pcd::cloud_info>("mapping_pcd/feature/cloud_info", 10, &mapOptimization::laserCloudInfoHandler, this, ros::TransportHints().tcpNoDelay());
        subGPS = nh.subscribe<nav_msgs::Odometry> (gpsTopic, 200, &mapOptimization::gpsHandler, this, ros::TransportHints().tcpNoDelay());

        //added ******************by gc
        subIniPoseFromRviz = nh.subscribe("/initialpose", 8, &mapOptimization::initialpose_callback, this);
        pubMapWorld = nh.advertise<sensor_msgs::PointCloud2>("mapping_pcd/mapping/cloud_map_map",1);//
        //fortest_publasercloudINWorld = nh.advertise<sensor_msgs::PointCloud2>("mapping_pcd/mapping/laserclouinmapframe",1);
        pubLaserCloudInWorld = nh.advertise<sensor_msgs::PointCloud2>("mapping_pcd/mapping/lasercloud_in_world", 1);//added
        pubOdomToMapPose = nh.advertise<geometry_msgs::PoseStamped>("mapping_pcd/mapping/pose_odomTo_map", 1);
...
#pragma omp parallel for num_threads(numberOfCores)
    ...
    {
        //Load file pcd
        pcl::io::loadPCDFile(std::getenv("HOME") + loadmapDirectory + "GlobalMap.pcd", *cloudGlobalMap);

        //Map_global thành local
        std::ifstream map_project_file(std::getenv("HOME") + loadmapDirectory + "map_projector.project");
        if (map_project_file.is_open()) {
        std::string tieu_de;
        double X0;
        double Y0;
        double Z0;
        map_project_file >> tieu_de >> X0 >> Y0 >> Z0;
        for(auto& pt : cloudGlobalMap->points) {
            pt.getVector3fMap() -= Eigen::Vector3f(X0, Y0, Z0);
        }
        std::cout << "Toa do Global offset (x = "
                        << X0 << ", y = " << Y0 << ") và do cao (z = " << Z0 << ")" << std::endl;
        }
        ...
        *cloudGlobalMapDS = *cloud_temp;  //Luu gia tri ban do sau filter

        std::cout << "Kich thuoc global map: " << cloudGlobalMap->points.size() << std::endl;
        std::cout << "Kich thuoc global map after filter: " << cloudGlobalMapDS->points.size() << std::endl;

	    publishCloud(&pubMapWorld, cloudGlobalMapDS, ros::Time::now(), "map");
    }
	...
	//cloudScanForInitialize->clear();
        ...
        if (icp.hasConverged() == false || icp.getFitnessScore() > historyKeyframeFitnessScore_RS)
        {
...
//        std::cout << "the size of input cloud: " << latestCloudIn->points.size() <<std::endl;
	...
	//Eigen::Affine3f transodomToWorld_init = pcl::getTransformation(0,0,0,0,0,0);
        ...
        if (icp.hasConverged() == true && icp.getFitnessScore() < historyKeyframeFitnessScore_RS)
        {
            geometry_msgs::PoseStamped pose_odomTo_map;
