...
public:
    ...
    // Eigen::MatrixXd getLastISCRGB(void);
    cv::Mat getLastISCRGB_PC(void);
    cv::Mat getLastISCRGB_CC(void);
    Eigen::MatrixXd makeScancontext( pcl::PointCloud<SCPointType> & _scan_down );
    Eigen::MatrixXd makeScancontext_cartcontext( pcl::PointCloud<SCPointType> & _scan_down );
    ...
    int detectLoopClosureID_intensity( void );

    // for ltmapper
    const Eigen::MatrixXd& getConstRefRecentSCD(void);

    bool is_loop_pair(MatrixXd& desc1, MatrixXd& desc2, double& geo_score, double& inten_score);
    double calculate_geometry_dis(const MatrixXd& desc1, const MatrixXd& desc2, int& angle);
    double calculate_intensity_dis(const MatrixXd& desc1, const MatrixXd& desc2, int& angle);
...
public:
    ...
    double PC_MAX_RADIUS = 80.0; // 80.0 for outdoor; 20.0-40.0 for indoor
    const double PC_UNIT_SECTORANGLE = 360.0 / double(PC_NUM_SECTOR);
    const double PC_UNIT_RINGGAP = PC_MAX_RADIUS / double(PC_NUM_RING);

    //Cart context
    const int CC_NUM_X = 40;
    const int CC_NUM_Y = 40;
    const int CC_MAX_X = 100;
    const int CC_MAX_Y = 100;
    ...
    // SC_DIST_THRES la thong so scancontext tot nhat

    // config
    const int    TREE_MAKING_PERIOD_ = 50; // i.e., remaking tree frequency, to avoid non-mandatory every remaking, to save time cost / in the LeGO-LOAM integration, it is synchronized with the loop detection callback (which is 1Hz) so it means the tree is updated every 10 sec. But you can use the smaller value because it is enough fast ~ 5-50ms wrt N.
    int          tree_making_period_conter = 0;

    float GEOMETRY_THRESHOLD = 0.67;
    float INTENSITY_THRESHOLD = 0.91;   //0.91
    ...
    // std::vector<Eigen::Vector3d> color_projection;
